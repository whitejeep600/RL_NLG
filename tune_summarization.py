...
class ModelArguments:
    ...
    decoder: Optional[str] = field(
        default=None,
        metadata={
            "help": "The decoding algorithm to use: greedy, beam_search, top_p or top_k",
        }
    )
...
def main():
    ...
    gen_kwargs = {}
    if model_args.decoder == 'greedy':
        gen_kwargs['num_beams'] = 1
        gen_kwargs['do_sample'] = False
    elif model_args.decoder == 'beam_search':
        assert model_args.beam_size is not None, 'Please provide a beam size for beam search decoding (--beam_size=x)'
        gen_kwargs['num_beams'] = model_args.beam_size
    elif model_args.decoder == 'top_p':
        assert model_args.p is not None, 'Please provide a p for top-p search decoding (--p=x)'
    ...
    if training_args.do_eval:
        ...
        metrics = trainer.evaluate(max_length=max_length, metric_key_prefix="eval", **gen_kwargs)
    ...
    if training_args.do_predict:
        ...
        with torch.no_grad():
            ...
            predict_results = trainer.predict(
                predict_dataset, metric_key_prefix="predict", max_length=max_length, **gen_kwargs
            )
