...
def parse_args():
    ...
    parser.add_argument('--decoder', type=str, choices=['greedy', 'beam_search', 'top_p', 'top_k'], default=None)
...
def main():
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        if args.decoder == 'greedy':
            ...
            assert args.beam_size is not None, 'Please provide a beam size for beam search decoding (--beam_size=x)'
            gen_kwargs['num_beams'] = args.beam_size
        elif args.decoder == 'top_p':
            assert args.p is not None, 'Please provide a p for top-p search decoding (--p=x)'
            gen_kwargs['top_p'] = args.p
        elif args.decoder == 'top_k':
            assert args.k is not None, 'Please provide a k for top-k search decoding (--k=x)'
            gen_kwargs['top_k'] = args.k
        else:
